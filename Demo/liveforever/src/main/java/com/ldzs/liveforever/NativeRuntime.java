...
public class NativeRuntime {
	 ...
	 * @return
	 */
	public String RunExecutable(String packageName, String filename, String alias, String args) {
		...
		if (!new File("/data/data/" + alias).exists()) {
			RunLocalUserCommand(packageName, cmd4, sb_result); // 拷贝lib/libtest.so到上一层目录,同时命名为test.
			sb_result.append(";");
		}
		RunLocalUserCommand(packageName, cmd3, sb_result); // 改变test的属性,让其变为可执行
		sb_result.append(";");
		RunLocalUserCommand(packageName, cmd2_a1, sb_result); // 执行test程序.
		sb_result.append(";");
	 ...
	 * @return
	 */
	public boolean RunLocalUserCommand(String packageName, String command, StringBuffer sb_out_Result) {
